...
function HeroWithConnector() {
  ...
  // Line starts immediately and grows steadily across the full scroll
  const lineWidth = useTransform(scrollYProgress, [0, 0.9], ['0%', '100%'])
  
  // Shapes move aggressively left+down to meet the line at ~75% mark
  const shapeX = useTransform(scrollYProgress, [0.1, 0.7], [0, -500])
  const shapeY = useTransform(scrollYProgress, [0.1, 0.7], [0, 180])
  const shapeScale = useTransform(scrollYProgress, [0.6, 0.8], [1, 0.5])
  const shapeOpacity = useTransform(scrollYProgress, [0.7, 0.85], [1, 0])
  ...
  return (
    ...
    <section className="sticky top-0 pt-20 md:pt-32 pb-10 relative overflow-hidden bg-paper min-h-screen z-10">
      ...
      {/* The connector line — inline */}
      <div className="max-w-7xl mx-auto px-6 mt-12 mb-2">
