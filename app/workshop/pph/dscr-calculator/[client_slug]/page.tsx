...
function calculateRateFromSheet(
  ...
  const match = rateSheet.rates.find(
    (r) =>
      ltv >= r.ltv_min &&
      ltv <= r.ltv_max &&
      fico >= r.fico_min &&
  ...
  return match.standard_rate + (isIO ? match.io_adjustment : 0)
}

// ─── Component ──────────────────────────────────────────────────────────────

export default function ClientDSCRCalculatorPage() {
  ...
  // Auto-calculate best interest rate from rate sheet
  const interestRate = useMemo(() => {
    if (!config) return 7.5
    return calculateRateFromSheet(
      ltv,
      ficoScore,
      amortization === 'io',
      config.rate_sheet_data
    )
  }, [config, ltv, ficoScore, amortization])
  ...
  const qualification = useMemo(() => {
    ...
    if (dscr >= 1.00) return {
      status: 'marginal',
      label: 'Marginal — May qualify with compensating factors',
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-cream">
        <div className="text-center">
          <div className="w-8 h-8 border-2 border-midnight/20 border-t-ocean rounded-full animate-spin mx-auto mb-4" />
          <p className="font-display text-xl text-midnight">Loading calculator...</p>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-cream px-6">
        <div className="max-w-md text-center">
          <div className="w-16 h-16 bg-midnight/5 rounded-2xl flex items-center justify-center mx-auto mb-6">
            <span className="text-3xl">🏠</span>
          </div>
          <h1 className="font-display text-3xl text-midnight mb-3">Page Not Found</h1>
          <p className="text-midnight/60 mb-8">
            We couldn&apos;t find a calculator configuration for this link. It may have been removed or the URL may be incorrect.
          </p>
          <Link
            href="/workshop/pph/dscr-calculator"
            className="inline-flex items-center gap-2 bg-midnight text-cream px-6 py-3 rounded-xl font-medium hover:bg-midnight/90 transition-colors"
          >
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" />
            </svg>
            Go to DSCR Calculator
          </Link>
          <p className="text-xs text-midnight/40 mt-8">Kanons</p>
        </div>
      </div>
    )
  }
    ...
    <div className="max-w-6xl mx-auto pb-24">
      ...
      <div className="mb-8">
        ...
        <p className="text-midnight/60 text-lg">
          Prepared for <span className="font-medium text-midnight">{config.client_name}</span>
          {config.client_fico && <span> &middot; FICO {config.client_fico}</span>}
        </p>

        {/* LO Contact Info */}
        <div className="mt-4 flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-midnight/60">
          <span className="font-medium text-midnight">{config.lo_name}</span>
          <span>NMLS #{config.lo_nmls}</span>
          <a href={`tel:${config.lo_phone}`} className="hover:text-ocean transition-colors">{config.lo_phone}</a>
          <a href={`mailto:${config.lo_email}`} className="hover:text-ocean transition-colors">{config.lo_email}</a>
        </div>
      </div>
            ...
            <div className="space-y-6">
              ...
              <div className="bg-ocean/5 rounded-lg p-4 border border-ocean/10">
                ...
                <div className="text-xs text-midnight/50 mt-1">
                  {formatCurrency(purchasePrice)} &times; {100 - downPaymentPercent}%
                </div>
              </div>

              {/* Interest Rate (Auto-calculated, display only) */}
              <div>
                <label className="block text-sm font-medium text-midnight mb-2">
                  Interest Rate (from rate sheet)
                </label>
                <div className="bg-ocean/5 rounded-lg px-4 py-3 border border-ocean/10">
                  <span className="font-display text-2xl text-midnight">{interestRate.toFixed(3)}%</span>
                  <span className="text-xs text-midnight/50 ml-2">
                    Based on {ltv.toFixed(0)}% LTV &middot; {ficoScore} FICO
                    {amortization === 'io' && ' · I/O'}
                  </span>
                </div>
              </div>
            ...
            <div className="space-y-6">
              ...
              <div>
                <label className="block text-sm font-medium text-midnight mb-2">
                  ...
                  {config.client_fico && ficoScore === config.client_fico && (
                    <span className="text-xs text-ocean ml-2">(pre-filled)</span>
                  )}
                </label>
                <Slider
                  value={ficoScore}
          ...
          <div className="sticky top-6">
            <div className={`rounded-2xl border-2 ${qualification.border} ${qualification.bg} p-6 shadow-lg`}>
              ...
              <div className="mb-6">
                <div className="h-3 bg-midnight/10 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ width: `${Math.min(Math.max(dscr / 1.5 * 100, 0), 100)}%`, transition: 'width 0.3s' }}
                  />
                </div>
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center py-2 border-b border-midnight/10">
                  <span className="text-xs text-midnight/60">Interest Rate</span>
                  <span className="font-medium text-sm text-ocean">
            ...
            <div className="mt-6 bg-cream rounded-2xl border-2 border-midnight/5 p-6">
              ...
              <div className="space-y-4 text-sm">
                <div>
                  ...
                  <div className="font-mono text-xs bg-midnight/5 rounded px-2 py-1">
                    {formatCurrency(grossMonthlyRent)} &times; {vacancyFactor}% = {formatCurrency(effectiveMonthlyRent)}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="font-mono text-xs bg-midnight/5 rounded px-2 py-1 mb-1">
                    {formatCurrency(effectiveMonthlyRent)} &divide; {formatCurrency(monthlyPITIA)}
                  </div>
      ...
      {/* Info Footer */}
      <div className="mt-12 bg-midnight/5 rounded-xl p-6">
        <h3 className="font-display text-lg text-midnight mb-3">About DSCR Loans</h3>
        <div className="text-sm text-midnight/70 space-y-2">
          ...
          <p>
            <strong>Typical Requirements:</strong> DSCR &ge; 1.15-1.25 (varies by lender), FICO &ge; 680, 15-25% down payment. Rates are typically 0.5-1.5% higher than conventional mortgages.
          </p>
          <p className="text-xs text-midnight/50">
            This calculator provides estimates for planning purposes only. Actual qualification depends on lender guidelines, credit profile, and property specifics. Contact your loan officer for personalized guidance.
          </p>
        </div>
      </div>

      {/* Powered by LO Footer */}
      <div className="mt-8 text-center pb-8">
        <div className="inline-flex flex-col items-center gap-2">
          <div className="h-px w-16 bg-midnight/10" />
          <p className="text-sm text-midnight/50">
            Powered by <span className="font-medium text-midnight/70">{config.lo_name}</span>
          </p>
          <p className="text-xs text-midnight/40">
            NMLS #{config.lo_nmls} &middot;{' '}
            <a href={`tel:${config.lo_phone}`} className="hover:text-ocean transition-colors">{config.lo_phone}</a> &middot;{' '}
            <a href={`mailto:${config.lo_email}`} className="hover:text-ocean transition-colors">{config.lo_email}</a>
          </p>
        </div>
      </div>
